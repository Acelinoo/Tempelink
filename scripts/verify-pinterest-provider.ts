...
async function verifyPinterest() {
  console.log('======================================================');
  console.log('TEMPELINK — PINTEREST PROVIDER VERIFICATION');
  console.log('======================================================');

  const apiKey = serverConfig.pinterest.apiKey;
  console.log(
    `[CONFIG] API Key Status: ${apiKey ? 'CONFIGURED' : 'NOT CONFIGURED (PINTEREST_PROVIDER_API_KEY is empty in .env.local)'}`
  );
  console.log(`[CONFIG] Base URL: ${serverConfig.pinterest.baseUrl}`);
  ...
  if (detection.status !== 'SUPPORTED_PLATFORM') {
    console.error('[FAILED] URL is not detected as a supported Pinterest pin.');
    process.exit(1);
  }
  ...
  if (!apiKey) {
    ...
    console.log(
      'Notice: PINTEREST_PROVIDER_API_KEY is not yet configured in .env.local.'
    );
    console.log(
      'To verify live Pinterest resolution, configure PINTEREST_PROVIDER_API_KEY with your authorized RapidAPI key.'
    );
    return;
  }

  console.log('\n[STEP 2] Calling live upstream Pinterest API...');
  try {
    ...
    console.log('\n======================================================');
    console.log('PINTEREST LIVE RESOLUTION VERIFIED SUCCESSFULLY!');
    console.log('======================================================');
  } catch (err: any) {
